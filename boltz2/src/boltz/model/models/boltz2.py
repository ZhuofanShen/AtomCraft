...
class Boltz2(LightningModule):
    ...
    def get_distogram_confidence(
        self,
        feats: dict[str, Tensor],
        recycling_steps: int = 0, ##TODO: Remove
            ...
            dict_out.update(
                self.confidence_module(
                    ...
                    x_pred=(
                        ...
                        else feats_["coords"].repeat_interleave(diffusion_samples, 0)
                    ),
                    feats=feats_,
    ...
    def forward(
        ...
        binder_chain: str = "A",
        logmd: bool  = False,
        multi_target_binder: bool = False,
        structure: Any = None,
        align_targets_together: bool = False,
        run_pairformer_only: bool = False,
        run_diffusion_only: bool = False,
        dict_out: dict[str, Tensor] = None,
        tie_targets: bool = False,
    ) -> dict[str, Tensor]:

        # DEBUG multi targets 
        if multi_target_binder:
            entity_id= feats["entity_id"]
            is_binder = entity_id == chain_to_number[binder_chain]
            is_target = entity_id != chain_to_number[binder_chain]
            same_chain = entity_id[:, :, None] == entity_id[:, None, :]
            binder_links = is_binder[:, :, None] | is_binder[:, None, :]
            target_pair_mask = same_chain | binder_links
            feats["target_pair_mask"] = target_pair_mask
        
        else:
            feats["target_pair_mask"] = None
        ...
        with torch.set_grad_enabled(
            ...
            if not run_diffusion_only:
                ...
                if randomly_kill_helix_feature:
                    print("Randomly kill signal")
                    seq_len = s.shape[1]
                    binder_mask = feats["entity_id"] == chain_to_number[binder_chain]
                    for i in range(seq_len-4):
                        if binder_mask[0, i] == True:
                            # random_factor = 0.1 *torch.rand(1).item() * 0.5 - 0.1  # Random value between -0.1 and 0.1
            ...
            if (
                ...
                with torch.autocast("cuda", enabled=False):
                    struct_out = self.structure_module.sample(
                        ...
                        align_targets_together=align_targets_together
                    )
                    dict_out.update(struct_out)
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0, multi_target_binder: bool = False, randomly_kill_helix_feature: bool = False, negative_helix_constant: float = 0.1, binder_chain: str = "A", logmd: bool = False, structure: Any = None, align_targets_together: bool = False, tie_targets: bool = False, run_pairformer_only: bool = False, run_diffusion_only: bool = False, dict_out: dict[str, Tensor] = None) -> dict:
        try:
            out = self(
                batch,
                ...
                logmd = logmd,
                multi_target_binder=multi_target_binder,
                structure = structure,
                align_targets_together=align_targets_together,
                run_pairformer_only=run_pairformer_only,
                run_diffusion_only=run_diffusion_only,
                dict_out=dict_out,
                tie_targets=tie_targets,
            )
