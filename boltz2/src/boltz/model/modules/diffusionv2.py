...
class DiffusionModule(Module):
    ...
    def forward(
        ...
        a, q_skip, c_skip, to_keys = self.atom_attention_encoder(
            ...
            target_pair_mask=feats["target_pair_mask"],
        )
...
class AtomDiffusion(Module):
    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        # -----------------------------------------------------------------
        # START: *** NEW TYING LOGIC ***
        # -----------------------------------------------------------------
        feats = network_condition_kwargs["feats"]
        num_targets = feats["entity_id"].shape[0]

        if tie_targets:
            
            try:
                atom_to_token = feats["atom_to_token"] # [1, N_atoms, N_tokens]
                entity_id_per_token = feats["entity_id"] # [1, N_tokens]

                # Ensure correct dimensions (remove batch dim 0)
                atom_to_token = atom_to_token.squeeze(0) # [N_atoms, N_tokens]
                entity_id_per_token = entity_id_per_token.squeeze(0) # [N_tokens]

                entity_id_one_hot = F.one_hot(
                    entity_id_per_token.long(), int(entity_id_per_token.max()) + 1
                ).float() # [N_tokens, N_entities]
                
                # Perform matmul: [N_atoms, N_tokens] @ [N_tokens, N_entities] -> [N_atoms, N_entities]
                entity_id_per_atom_logits = atom_to_token.float() @ entity_id_one_hot 
                entity_id_per_atom = entity_id_per_atom_logits.argmax(-1)  # [N_atoms]

                chain0_mask = (entity_id_per_atom == 0) # Shape [N_atoms]
                token_pad_mask = feats["atom_pad_mask"].bool()
                chain0_mask = chain0_mask & token_pad_mask
                chain0_mask =chain0_mask[0]

            except Exception as e:
                print(f"Error creating 'chain0_mask' for tying: {e}. Skipping tying.")
                tie_targets = False

        def tie_tensor(tensor):
            """Averages pairs of batches for chain_id 0."""
            if not tie_targets or tensor is None:
                return tensor

            num_pairs = num_targets // 2
            
            n_chain0 = chain0_mask.sum().item()
            chain0_tensor = tensor[:, chain0_mask, :] # Shape (Multiplicity, N_chain0, 3)
            # Reshape into pairs: (num_pairs, 2, N_chain0, 3)
            
            paired_tensor = chain0_tensor.view(num_pairs, 2, n_chain0, 3)
            
            # Compute mean: (num_pairs, 1, N_chain0, 3)
            mean_tensor = paired_tensor.mean(dim=1, keepdim=True)
            
            # Expand: (num_pairs, 2, N_chain0, 3)
            tied_tensor = mean_tensor.expand(num_pairs, 2, n_chain0, 3)
            # Reshape back: (Multiplicity, N_chain0, 3)
            tied_tensor_flat = tied_tensor.view(num_targets, n_chain0, 3)
            # Create a copy and apply the change
            tensor_out = tensor.clone() 
            tensor_out[:, chain0_mask, :] = tied_tensor_flat
            return tensor_out
        ...
        for step_idx, (sigma_tm, sigma_t, gamma) in enumerate(sigmas_and_gammas):
            ...
            with torch.no_grad():
                ...
                if tie_targets or num_targets >1:
                    sample_ids = torch.arange(num_targets).to(atom_coords_noisy.device)
                    sample_ids_chunks = sample_ids.chunk(
                        num_targets % max_parallel_samples + 1
                    )
                    ...
                    if tie_targets:
                        atom_coords_denoised = tie_tensor(atom_coords_denoised)
            ...
            if align_targets_together:
                atom_to_token = network_condition_kwargs["feats"]["atom_to_token"].squeeze(0)  # [1, N_atoms, N_tokens]
                entity_id_per_token = network_condition_kwargs["feats"]["entity_id"].squeeze(0)  # [1, N_tokens]
                atom_backbone_feat = network_condition_kwargs["feats"]["atom_backbone_feat"].squeeze(0)  # [1, N_atoms, ?]

                # --- Map per-atom entity IDs ---
                entity_id_one_hot = F.one_hot(
                    entity_id_per_token.long(), int(entity_id_per_token.max()) + 1
                ).float()
                entity_id_per_atom = (atom_to_token.float() @ entity_id_one_hot).argmax(-1)  # [1, N_atoms]

                # --- Identify chains ---
                chain_1_id = 1  # B
                chain_2_id = 2  # C
                is_chain_1 = (entity_id_per_atom == chain_1_id)
                is_chain_2 = (entity_id_per_atom == chain_2_id)

                # --- Identify Cα atoms ---
                is_calpha = (atom_backbone_feat.argmax(dim=-1) == 2)
                mask_1_ca = (is_chain_1 & is_calpha)
                mask_2_ca = (is_chain_2 & is_calpha)
                # --- Squeeze to 1D masks ---
                mask_1_ca_1d = mask_1_ca.squeeze(0)
                mask_2_ca_1d = mask_2_ca.squeeze(0)
                is_chain_1_1d = is_chain_1.squeeze(0)
                is_chain_2_1d = is_chain_2.squeeze(0)


                # --- Extract CA coordinates ---
                coords_1_ca = atom_coords_noisy[0, mask_1_ca_1d]
                coords_2_ca = atom_coords_noisy[0, mask_2_ca_1d]


                # --- Handle unequal CA counts ---
                n = min(coords_1_ca.shape[0], coords_2_ca.shape[0])
                if n < 3:
                    print("[WARN] Not enough CA atoms to align (need ≥3). Skipping alignment.")
                else:
                    coords_1_ca = coords_1_ca[:n]
                    coords_2_ca = coords_2_ca[:n]

                    # --- Compute rigid-body transform (align 2 → 1) ---
                    center_1 = coords_1_ca.mean(dim=0, keepdim=True)
                    center_2 = coords_2_ca.mean(dim=0, keepdim=True)
                    coords_1_centered = coords_1_ca - center_1
                    coords_2_centered = coords_2_ca - center_2

                    H = coords_2_centered.T @ coords_1_centered
                    U, S, Vt = torch.linalg.svd(H)
                    R = Vt.T @ U.T
                    if torch.det(R) < 0:
                        Vt[-1, :] *= -1
                        R = Vt.T @ U.T
                    t = center_1 - center_2 @ R.T

                    # --- Apply rigid transform to all atoms of chain 2 ---
                    coords_all_2 = atom_coords_noisy[0, is_chain_2_1d]  # [n_atoms_2, 3]
                    coords_all_2_aligned = coords_all_2 @ R.T + t

                    # --- Write back aligned coordinates ---
                    atom_coords_aligned = atom_coords_noisy.clone()
                    atom_coords_aligned[0, is_chain_2_1d] = coords_all_2_aligned
                    atom_coords_noisy = atom_coords_aligned 
                    


            if tie_targets:
                binder_1_coords = atom_coords_denoised[0, chain0_mask]
                binder_2_coords = atom_coords_denoised[1, chain0_mask]

                center_1 = binder_1_coords.mean(dim=0, keepdim=True)
                center_2 = binder_2_coords.mean(dim=0, keepdim=True)

                binder_1_coords = binder_1_coords - center_1
                binder_2_coords = binder_2_coords - center_2
                
                H = binder_2_coords.T @ binder_1_coords
                U, S, Vt = torch.linalg.svd(H)
                R = Vt.T @ U.T
                if torch.det(R) < 0:
                    Vt[-1, :] *= -1
                ...
                atom_coords_noisy[1] = atom_coords_noisy[1] @ R.T + t
                atom_coords_denoised[1] = atom_coords_denoised[1] @ R.T + t
