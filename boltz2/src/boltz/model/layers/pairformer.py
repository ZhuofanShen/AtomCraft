...
class PairformerLayer(nn.Module):
    ...
    def forward(
        ...
        mask: Tensor,
        pair_mask: Tensor,
        target_pair_mask: Optional[Tensor] = None,
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            s = s.float() + self.attention(
                s=s_normed, z=z.float(), mask=mask.float(), k_in=s_normed, target_pair_mask=target_pair_mask
            )
    ...
    def forward(
        ...
        for layer in self.layers:
            if self.activation_checkpointing and self.training:
                ...
                s, z = layer(s, z, mask, pair_mask, target_pair_mask, chunk_size_tri_attn, use_kernels)
        return s, z
